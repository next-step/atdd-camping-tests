...
tasks.register("SetupReservationRepo") {
    group = "setup reservation"
    description = "Clones or pulls the atdd-camping-reservation repository and checks out parkSeryu branch."

    doLast {
        setupRepository(
            project,
            "https://github.com/ParkSeryu/atdd-camping-reservation.git",
            serviceDir
        )
    }
}
...
tasks.register("SetupReservation") {
    group = "setup reservation"
    description = "Sets up the atdd-camping-reservation by cloning/pulling and checking out the parkSeryu branch."
    dependsOn("SetupReservationRepo")
    doLast {
